...
def main(args):
    ...
    for j in range(1, 2):
        ...
        print("./place_fastorder_multi.sh CHTA USDT-PLG20 {} {} | jq '.'".format((CHTA_USDT_price * (1 + spread)), CHTA_unit))
        result = subprocess.run("./place_fastorder_multi.sh CHTA USDT-PLG20 {} {} | jq '.'".format((CHTA_USDT_price * (1 + spread)), CHTA_unit), shell=True)
        print("./place_fastorder_multi.sh USDT-PLG20 CHTA {} {} | jq '.'".format((USDT_CHTA_price * (1 + spread)), USDT_unit))
        result = subprocess.run("./place_fastorder_multi.sh USDT-PLG20 CHTA {} {} | jq '.'".format((USDT_CHTA_price * (1 + spread)), USDT_unit), shell=True)

        print("./place_fastorder_multi.sh NENG USDT-PLG20 {} {} | jq '.'".format((NENG_USDT_price * (1 + spread)), NENG_unit))
        result = subprocess.run("./place_fastorder_multi.sh NENG USDT-PLG20 {} {} | jq '.'".format((NENG_USDT_price * (1 + spread)), NENG_unit), shell=True)
        print("./place_fastorder_multi.sh USDT-PLG20 NENG {} {} | jq '.'".format((USDT_NENG_price * (1 + spread)), USDT_unit))
        result = subprocess.run("./place_fastorder_multi.sh USDT-PLG20 NENG {} {} | jq '.'".format((USDT_NENG_price * (1 + spread)), USDT_unit), shell=True)

        print("./place_fastorder_multi.sh CHTA USDC-PLG20 {} {} | jq '.'".format((CHTA_USDC_price * (1 + spread)), CHTA_unit))
        result = subprocess.run("./place_fastorder_multi.sh CHTA USDC-PLG20 {} {} | jq '.'".format((CHTA_USDC_price * (1 + spread)), CHTA_unit), shell=True)
        print("./place_fastorder_multi.sh USDC-PLG20 CHTA {} {} | jq '.'".format((USDC_CHTA_price * (1 + spread)), USDC_unit))
        result = subprocess.run("./place_fastorder_multi.sh USDC-PLG20 CHTA {} {} | jq '.'".format((USDC_CHTA_price * (1 + spread)), USDC_unit), shell=True)

        print("./place_fastorder_multi.sh NENG USDC-PLG20 {} {} | jq '.'".format((NENG_USDC_price * (1 + spread)), NENG_unit))
        result = subprocess.run("./place_fastorder_multi.sh NENG USDC-PLG20 {} {} | jq '.'".format((NENG_USDC_price * (1 + spread)), NENG_unit), shell=True)
        print("./place_fastorder_multi.sh USDC-PLG20 NENG {} {} | jq '.'".format((USDC_NENG_price * (1 + spread)), USDC_unit))
        result = subprocess.run("./place_fastorder_multi.sh USDC-PLG20 NENG {} {} | jq '.'".format((USDC_NENG_price * (1 + spread)), USDC_unit), shell=True)
    ...
    for row in rows:
        ...
        if not is_active_order:
            ...
            for trade in filled_order_list:
                if ((float(trade['quantity']) == quant_4f) and (trade['side'] == row['arb_side'])):
                    print("filled order cleared: {}  quant_4f: {} arb_side: {}".format(trade, str(quant_4f), row['arb_side']))
                    hedge_side = flip_side(row['arb_side'])
                    insert_net_unhedged_record(dbconn2,row['coin'], hedge_side, row['quantity']);
                    update_remainderswap_table(dbconn2,row['coin'], row['quantity'], row['arb_price'], row['arb_side'], 2)
