...
def main(args):
    if args.total is None and args.total_usd is None:
        assert False, "--total or --total_usd must be specified for streaming swap trade"
    
    ## print (args.total , args.total_usd)
    current_prices = get_prices()
    USD_unit = args.usd_unit
    base_spread = args.slippage
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--total', type=float, nargs='?',
                        help='total - total trading amount on the coin to be traded')
    parser.add_argument('-tusd', '--total_usd', type=float, nargs='?',
                        help='total amount in USD - total USD worth of trading amount on the coin to be traded using latest market price')
    parser.add_argument('-s', '--side', type=str, action='store', choices=['buy','sell'], required = True,
                      help="My trade side (buy or sell)")
    parser.add_argument('-m', '--market', type=str, action='store', required = True,
                      help="Market symbol, eg. NENG/KMD CHTA/KMD NENG/DGB-segwit CHTA/USDT-PLG20 etc")
    parser.add_argument('--usd_unit', type=float, nargs='?', default=0.5 , 
                        help='USD_unit - individual streaming swap trading amount on USD worth, [default: 0.5]')
